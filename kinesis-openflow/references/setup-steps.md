...
- Design columns based on your data fields — use VARCHAR for all initially
- The connector maps JSON field names to Snowflake column names (case-insensitive match)
...
Every column in the table must have a matching field in the source JSON, or you'll get
a `SchemaMismatchException`.

**Option A — OpenSky test data:**

Schema derived from actual OpenSky API response fields. All columns are VARCHAR
because the source JSON sends numeric values as strings (e.g., `"alt": "23275"`).
...
-- Flight data schema from OpenSky (all 8 fields from source)
...
CREATE TABLE <DB_NAME>.PUBLIC.<TABLE_NAME> (
    UTC VARCHAR,        -- Unix epoch timestamp (string from source)
    ID VARCHAR,         -- Flight callsign (e.g., DAL1742)
    ICAO VARCHAR,       -- Aircraft ICAO hex code
    ORIG VARCHAR,       -- Origin airport ICAO code (e.g., KATL)
    DEST VARCHAR,       -- Destination airport ICAO code (e.g., KSJC)
    ALT VARCHAR,        -- Altitude in feet (string from source)
    LAT VARCHAR,        -- Latitude (string from source)
    LON VARCHAR         -- Longitude (string from source)
);
...
1. Fetch a sample record from your Kinesis stream:
   ```bash
   ...
   aws kinesis get-records --shard-iterator "$ITER" --limit 1 \
     --query 'Records[0].Data' --output text | base64 -d | python3 -m json.tool
   ```
2. Map each JSON field to a Snowflake column. Use VARCHAR for all fields
   initially — you can always cast or add typed columns later.
3. Do NOT add DEFAULT values, AUTOINCREMENT, or GEO columns (Snowpipe Streaming limitation).
4. Do NOT add an `INGESTED_AT` column with a DEFAULT — if you want ingestion
   tracking, add it as a plain column and let the connector populate it.
...
**For legacy connector** (`spcs1` runtime):
...
**For modularized connector** (`spcs1-regression-test` or newer runtimes):

```bash
# Deploy modularized connector from registry
<SKILL_DIR>/venv/bin/nipyapi --profile <OPENFLOW_PROFILE> ci deploy_flow \
  --registry_client ConnectorFlowRegistryClient \
  --bucket connectors \
...
**For legacy connector** (`kinesis` flow — single PG, single `configure_inherited_params` call):
...
**For modularized connector** (`kinesis-json-modularized` flow — 3 sub-PGs, separate config steps):
...
curl -s -X POST \
  ...
  "<NIFI_URL>/parameter-contexts/<SOURCE_PARAM_CONTEXT_ID>/update-requests" \
  -d '{
    "revision": { "version": 0 },
    "id": "<SOURCE_PARAM_CONTEXT_ID>",
    "component": {
      "id": "<SOURCE_PARAM_CONTEXT_ID>",
      "parameters": [
